...
def display_trialsleaderboard(week):
    try:
        trials_leaderboard = get_trials_leaderboard(week)
        solo_leaderboard = trials_leaderboard["payload"]["world"]["solo"]["all"]["entries"]
        solo_leaderboard = solo_leaderboard[:5]
        ...
        if trials_leaderboard:
            ...
            for i, guild_info in enumerate(leaderboard):
                ...
                draw.text((x+10, y+28), f"{guild_info['platform_name']}", fill=fill,font=font)
                ...
                # Draw remaining_time
                remaining_time = convert_to_time(guild_info['completion_time'])
